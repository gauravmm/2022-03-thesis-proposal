...
A leading example of this work is COF-PAC \cite{zhang2020provably}, which uses ridge regularized versions of GTD2 \cite{sutton2009fast} to learn both the value and emphasis models. The authors describe RR as essential to their method, particularly when the target policy changes during learning. This reliance on RR makes COF-PAC vulnerable to model bias. We illustrate this with a counterexample in which COF-PAC learns correctly without RR but diverges with RR.

\todo{rewrite this to make sense without the math. }
The error landscape of our example is illustrated in Figure~\ref{fig:emphval}. The unregularized distribution $\upsilon(0)$ is in a stable region of the curve, and consequently has low error. Regularizing the emphasis model moves the value model to $\upsilon(2\cdot 10^{-4})$, which lies in an unstable region of the error landscape. In this way, regularizing the emphasis function causes the learned value function to have large error.

More formally, we compute the value function error $d(\upsilon(\eta_m), \eta_v)$ with possibly different RR parameters for emphasis ($\eta_m$) and value ($\eta_v$). First, under COF-PAC without regularization, the value function error is $d(\upsilon(0), 0) = 8\cdot 10^{-4}$, well within the representation error. This means that unregularized COF-PAC corrects for off-policy sampling and learns the true value function. Regularizing only the emphasis function yields $d(\upsilon(2\cdot 10^{-4}), 0) = 418$, which is huge. Adding RR to the value function improves this, but the error is still orders of magnitude larger than the representation error: $d(\upsilon(2\cdot 10^{-4}), 0) = 3.00$. RR causes the value function to diverge, and this cannot be fixed by regularizing the value function separately. This completes the example.

Intuitively, there is a fundamental bootstrapping issue in this situation: we use an emphasis models because the value function may not converge to a good solution under TD, but to use the emphasis model we have to assume it converges to a good solution under TD. As this paper shows, the use of RR is not sufficient to guarantee good behavior.

\begin{figure}
  \input{emphasistd/emphasistd.tex}
  \caption{Ridge regularization distorts the emphasis model (left), which induces the value function (right) to move to a singularity. Unregularized models are shown in red, regularized models in blue. Ridge regularization can interact with emphasis models to significantly worsen learned value functions. }
