...
\subsubsection{Validating our assumptions}

We have made several assumptions in the construction of this algortihm which we must validate. Perhaps most importantly, we assume that $Z$ is low-rank.

\paragraph{optimal $Z$ is low-rank vs. some low-rank $Z$ exists} We assume that the optimal $Z$ is low-rank

Distinguish between these two claims. We make the former claim which is strictly stronger than the latter. 
...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \item etc.
    \item Is the convex subset a sufficiently expressive characterization of the algorithm.
  \end{enumerate}
\end{enumerate}
...
\end{align}
Using the Lagrange multiplier $Z\in\mathbb^{2k\times 2k}, we solve the inner optimization problem:
\begin{align}
  \underset{q}{\text{minimize}} - H(q) - \E_{s\sim q}[\log \mu(s) + \tr Z^\top F(s)]
\end{align}
\todo{Write down Lagrangian, find optima.}
\begin{align}
